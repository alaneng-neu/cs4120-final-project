...
def main():
    ...
    # Load best model
    if os.path.exists('best_bert_model_state.bin'):
    ...
    evaluate_multiclass_model(
        y_true=y_true, 
        y_pred=y_pred, 
        dataset_name="Phishing BERT (Test Set)"
    )
    ...
    # Save the model
    model_save_path = os.path.join(models_dir, "phishing_bert_model")
    model.save_pretrained(model_save_path)
    tokenizer.save_pretrained(model_save_path)
    print(f"Saved BERT model and tokenizer to {model_save_path}")
    
    # Save label encoder
    encoder_path = os.path.join(models_dir, "phishing_bert_encoder.joblib")
    joblib.dump(label_encoder, encoder_path)
    print(f"Saved label encoder to {encoder_path}")
    
    # Clean up temp file
    if os.path.exists('best_bert_model_state.bin'):
        os.remove('best_bert_model_state.bin')
